...
def setup_env():
    ...
    with closing(sqlite3.connect('/config/brokerr.db')) as connection:
        with closing(connection.cursor()) as cursor:
            ...
            cursor.execute("CREATE TABLE IF NOT EXISTS trackers(tracker TEXT PRIMARY KEY, last_insured INTEGER, num_insured INTEGER)")
...
def create_tracker_directory(tracker: str):
    tracker_dir = f"/config/screenshots/{tracker}"
    if not os.path.exists(tracker_dir):
        try:
            os.makedirs(tracker_dir)
        except OSError as e:
            print(f"Error creating directory '{tracker_dir}': {e}")
